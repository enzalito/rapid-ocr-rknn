...
std::vector<unsigned char> loadModelFile(const std::string &path) {
...
cv::Mat dequantizeOutput(const int8_t *data, int rows, int cols, int32_t zp, float scale) {
    // Wrap raw int8 data in a cv::Mat (no copy)
    cv::Mat quantized(rows, cols, CV_8SC1, const_cast<int8_t *>(data));
    cv::Mat result;
    // float_val = (quant_val - zp) * scale
    // convertTo: result = alpha * src + beta  =>  alpha=scale, beta=-zp*scale
    quantized.convertTo(result, CV_32F, scale, static_cast<double>(-zp) * scale);
    return result;
}
